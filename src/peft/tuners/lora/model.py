...
class LoraModel(BaseTuner):
    #通过低秩分解将权重更新表示为两个较小的矩阵（称为更新矩阵），从而加速大型模型的微调，并减少内存消耗
    #支持调用 LoRA 的层包括：nn.Linear、nn.Embedding、nn.Conv2d
    #前向计算：首先计算原参数与输入的乘积，再加上 A、B 分别与输入的乘积
    """
    ...
    Example:
        ...
        >>> config = LoraConfig(
        ...     task_type="SEQ_2_SEQ_LM",#为序列到序列语言建模
        ...     r=8,#低秩矩阵的维度
        ...     lora_alpha=32,#低秩矩阵的缩放因子
        ...     target_modules=["q", "v"],
        ...     lora_dropout=0.01,#LoRA层的dropout概率
        ... )
        ...
        #LoRA 微调参数target_modules，是需要进行 LoRA 的层名称
        #创建 LoRA 模型时，会获取该参数，然后在原模型中找到对应的层（使用 re 对层名进行正则匹配实现）
